...
| `radar` | Render | Draws a compact local overhead entity radar HUD. | `shape` (`circle`, `square`), `rotate`, `zoom`, `players`, `hostiles`, `passive`, `items`, `friends`, `maximum_entities`, `background_color`, `entity_color`, `friend_color` | Its enabled state and placement are local HUD-editor choices. It reads only currently loaded local entities, caps points at 512, and never reveals unloaded data. |
...
| `mini_player` | Render | Draws a local HUD miniature of the current player model. | `rotation`, `scale`, `armor`, `background_color` | Its enabled state and placement are local HUD-editor choices. Armor off clears only the temporary extracted render state; player inventory/equipment is never changed. |
...
| `speed` | Movement | Applies conservative local acceleration, strafe assistance, or multiplier motion. | `mode` (`vanilla_acceleration`, `strafe_assist`, `multiplier`), `multiplier`, `acceleration`, `maximum_speed` | All variants have a 0.60 local horizontal cap and a 2.0 multiplier ceiling; the default cap is 0.35 so gains above vanilla sprint are visible out of the box. Multiplayer servers can reject or correct movement; this module displays no anti-cheat presets. |
...
| `flight` | Movement | Ability flight where Minecraft permits it, local velocity flight elsewhere, boat flight, and an optional freecam view. | `flying_speed`, `survival_velocity`, `velocity_speed`, `boat_flight`, `boat_speed`, `freecam_view`, `freecam_speed` | With `mayfly` (creative/spectator/permitted) it uses normal abilities and restores only state Helikon changed. Without `mayfly`, `survival_velocity` sets ordinary local player velocity each tick (hover on no input, hold while a screen is open); `boat_flight` velocity-steers a locally driven ridden boat, including jump-key ascent. Both use only normal client movement updates — no packet manipulation and no anti-cheat bypass modes — so servers that enforce movement can reject, rubber-band, or kick; descent still accumulates normal fall distance, so combine with NoFall where permitted. Freecam creates an invisible client-only camera and suppresses player movement input. |
...
| `nuker` | World | Makes a bounded set of ordinary destroy requests for explicitly whitelisted loaded blocks. | `radius`, `blocks_per_tick`, `whitelist`, `blacklist`, `tool_selection`, `minimum_tool_durability`, `line_of_sight`, `rotate`, `safety_limit` | It is disabled by default and a blank whitelist is deliberately inert. It requires Attack, no screen, loaded in-range blocks, and caps requests at two per tick. With line of sight on, it ray-checks only the 32 nearest eligible blocks per tick. It uses Minecraft's normal destroy path and optional existing-hotbar selection; the server can reject attempts or correct client prediction. |
...
| --- | --- | --- | --- | --- |
| Active Modules | Lists enabled Helikon modules with registry, alphabetical, or width sorting. | `hud.json`: enabled state, position, scale, padding, backdrop/shadow, sort/alignment/color modes. | It is a local display only. | `ActiveModulesTest`, `HudEditorStateTest`, `HudConfigurationManagerTest`, manual HUD checklist. |
