...
| `anti_spam` | Chat | Locally suppresses duplicate, rapid, or burst join/leave incoming messages. | `stack_duplicates`, `hide_repeats`, `repeat_window_seconds`, `rapid_message_limit`, `rapid_window_seconds`, `collapse_join_leave`, `whitelisted_message_types` | It tracks at most 512 message identities and 256 sender windows in memory, and only affects this client HUD. The duplicate count is ready for a future display stacker; this slice can hide repeats but does not yet rewrite a visible chat line with a counter. |
